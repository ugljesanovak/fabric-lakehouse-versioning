...
/**
 * Calls the datahub wizard to select individual files from OneLake (not tables or folders)
 * Useful for file-based operations like adding files to repositories
 */
export async function callDatahubWizardOpenForFiles(    
    workloadClient: WorkloadClientAPI,
    supportedTypes: ExtendedItemTypeV2[],
    dialogSubmittButtonName: string,
    dialogDescription: string,    
    fileExtensions?: string[], // Optional: filter by file extensions (e.g., ['.csv', '.parquet'])
    multiSelectionEnabled: boolean = false,
    workspaceNavigationEnabled: boolean = true): Promise<ItemAndPath> {
   ...
   const datahubWizardConfig: DatahubWizardDialogConfig = {
        datahubCompactViewPageConfig: {
            datahubCompactViewConfig: {
                ...
                workspaceNavigationEnabled: workspaceNavigationEnabled,
                hostDetails: {
                    experience: 'LakehouseGitFS File Selection',
                    scenario: 'Add file to repository',
                }
            } as DatahubCompactViewConfig
        } as DatahubCompactViewPageConfig,
        oneLakeExplorerPageConfig: {
            ...
            onelakeExplorerConfig: {
                // Only show Files type, exclude Tables, Folders, Shortcuts
                onelakeExplorerTypes: [OnelakeExplorerType.Files],
    ...
    const selectedItem = result.onelakeExplorerResult;
    const { itemObjectId, workspaceObjectId } = selectedItem;
    const selectedPath = selectedItem.selectedPath.split('/').slice(2).join('/'); // Remove workspace and item segments
    
    // Optional: Validate file extension
    if (fileExtensions && fileExtensions.length > 0) {
        const fileExtension = selectedPath.toLowerCase().split('.').pop();
        if (!fileExtensions.some(ext => ext.toLowerCase().replace('.', '') === fileExtension)) {
            console.warn(`Selected file does not match allowed extensions: ${fileExtensions.join(', ')}`);
            // Still return it - let caller handle validation
        }
    }
    ...
    return {
        id: itemObjectId,
        workspaceId: workspaceObjectId,
        type: "Lakehouse", // Type of the container item
        displayName: "",
        description: "",
